...
const {
    ...
    storeRoute: "/mst_prc_supplier_category",
    updateRoute: "/mst_prc_supplier_category",
    deleteRoute: "/mst_prc_supplier_category",
    deleteMessage: "Hapus supplier category ini?",
});

const { search, entries, filters } = useTable({
    route: "/mst_prc_supplier_category",
...
<template>
    <AppLayout>
        <!-- Sesuaikan Halaman -->
        <Breadcrumb
            pageTitle="Supplier Category"
            :crumbs="['Master Data', 'Purchasing', 'Supplier Category']"
        />
        <DataTable
            :headers="[
            ...
            :from="supplierCategories.from"
            :to="supplierCategories.to"
        ...
        >
            ...
            <tr
                v-else
                v-for="supplierCategory in supplierCategories.data"
            ...
            >
                ...
                <td
                    class="border-r border-stroke px-4 py-5 text-center last:border-r-0 dark:border-strokedark"
                >
                    <StatusBadge :active="supplierCategory.is_active" />
                </td>
                ...
                >
                    <TableAction
                        @edit="openEdit(supplierCategory)"
                        @delete="openDelete(supplierCategory.id)"
                    />
                </td>
            </tr>

            <TableEmpty
                v-if="!loading && supplierCategories.data.length === 0"
                :colspan="5"
            />

            <template #pagination>
                <Pagination :links="supplierCategories.links" />
            </template>
        </DataTable>
        ...
        <Modal
            :show="showModal"
            :title="
                isEdit ? 'Edit Supplier Category' : 'Tambah Supplier Category'
            "
            @close="closeModal"
        >
            <!-- ... isi form modal Anda ... -->
