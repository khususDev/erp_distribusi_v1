...
const {
    ...
    storeRoute: "/mst_grl_location",
    updateRoute: "/mst_grl_location",
    deleteRoute: "/mst_grl_location",
    deleteMessage: "Hapus location ini?",
});

const { search, entries, filters } = useTable({
    route: "/mst_grl_location",
...
<template>
    <AppLayout>
        ...
        >
            ...
            <TableEmpty
                v-if="!loading && locations.data.length === 0"
                :colspan="6"
            />
        ...
        <Modal
            :show="showModal"
            :title="isEdit ? 'Edit Location' : 'Tambah Location'"
            @close="closeModal"
        >
            <!-- ... isi form modal Anda ... -->
